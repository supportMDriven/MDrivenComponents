...
function InvalidatePivot(thescope) {
  if (_therootpivot != undefined) {
    if (_therootpivotinvalidated)
      return;
    _therootpivotinvalidated = true;
    let thelatestscope = thescope;
    setTimeout(() => {
      RefreshPivot(_therootpivot);
      thelatestscope.$applyAsync(); //since RefreshPivot actually adds some data
